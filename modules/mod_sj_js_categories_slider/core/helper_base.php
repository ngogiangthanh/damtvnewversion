...
if (!class_exists('JsSliderBaseHelper')){
	/**
	 * BaseHelper for com_content only.
	 */
	abstract class JsSliderBaseHelper{
		/**
		...
		*/
		 ...
		 */
		public static function getJSAImage($item, $params, $prefix='imgcfg'){
			$images = &self::getJSAImages($item, $params, $prefix);
		 ...
		 * @param unknown_type $params
		 */
		public static function getJSAImages($item, $params, $prefix='imgcfg'){
			$hash = md5( serialize(array($params, 'article')) );
			if ( !isset(self::$image_cache[$hash][$item->product_id]) ){
				$defaults = array(
						'external'			=> 1,
						'image_product'		=> 1
				);
				...
				$images_path = array();
				$priority = preg_split('/[\s|,|;]/', $params->get($prefix.'_order', 'external, image_product'), -1, PREG_SPLIT_NO_EMPTY);
				if ( count($priority) > 0 ){
						...
						switch ($type){
							case 'image_product':
								if ($type == 'image_product'){
									if($item->product_name_image){
										$image_path = 'components/com_jshopping/files/img_products/'.$item->product_name_image;
									}else {
										$image_path = '';
									}
								...
								break;
							case 'external':
								$exf = $params->get($prefix.'_external_url', 'images/article/{product_id}/');
		...
		public static function getJSCImage($item, $params, $prefix='imgcfg'){
			$images = &self::getJSCImages($item, $params, $prefix);
			return is_array($images) && count($images) ? $images[0] : null;
		}

		public static function getJSCImages($item, $params, $prefix='imgcfg'){
			$hash = md5( serialize(array($params, 'category')) );
			if ( !isset(self::$image_cache[$hash][$item->category_id]) ){
				$defaults = array(
						'external'		=> 1,
						'image'		=> 1,
						'description'	=> 1
				...
				$images_path = array();
				$priority = preg_split('/[\s|,|;]/', $params->get($prefix.'_order', 'external, image, description'), -1, PREG_SPLIT_NO_EMPTY);
				if ( count($priority) > 0 ){
					$priority = array_map('strtolower', $priority);
					$mark = array();
						...
						switch ($type){
							case 'image':
								if($item->category_image != ''){
									$img_path = 'components/com_jshopping/files/img_categories/'.$item->category_image;
									$image = array(
											'src' => $img_path
									);
									array_push($images_path, $image);
				...
				self::$image_cache[$hash][$item->category_id] = $images_path;
			}
			return self::$image_cache[$hash][$item->category_id];
		}
